...
# Functions:
## `Dio_SetPinDirection(uint8_t port, uint8_t pinNum, uint8_t dir)`
...
void Dio_SetPinDirection(uint8_t port, uint8_t pinNum, uint8_t dir) {
  switch(port) {
    case 'A':
    case 'a':
      if(dir) {
...
## `Dio_SetPortDirection(uint8_t port, uint8_t dir)`
...
## `Dio_WritePort(uint8_t port, uint8_t state)`
...
uint8_t Dio_ReadPort(uint8_t port, uint8_t state) {
...
int main(void) {
  ...
  Dio_SetPortDirectionection('A', 0xFF);

  while(1) {
      Dio_TogglePort('A', 0xFF);
  }
}
```
